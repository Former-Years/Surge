...
echo "$lrr" | sed -e "s/RRAID/$1/g" > /jd/scripts/live_redrain.js

echo "红包雨RRA替换成$1完成"

# 删除旧的定时任务
sed -i '/live_redrain.js/d' /jd/config/crontab.list

echo "删除旧的live_redrain定时任务完成"

# 添加今天的定时任务
echo "0,30 0-23$(date "+ %-d %-m") * mtask live_redrain.js now"  >>  /jd/config/crontab.list
echo "0,30 0-23$(date "+ %-d %-m") * sleep 30; mtask live_redrain.js now"  >>  /jd/config/crontab.list

echo "添加当天的live_redrain红包雨定时任务完成"

echo "顺便白嫖一下红包雨"

mtask live_redrain now > /dev/null 
