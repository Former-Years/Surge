...
let Message = '';
let requestData = '';  // 存储请求的 URL
const KEY_erke_data = 'erke_data'; // 存储 URL 数据

// 判断运行环境
if (typeof $request !== 'undefined') {
    GetRequestBodyFromURL(); // 抓取 URL 数据
    $.done();
} else {
    !(async () => {
        loadRequestData(); // 加载抓取到的请求数据
        if (!requestData) {
...
// 从 URL 中抓取数据并存储
function GetRequestBodyFromURL() {
    if ($request && $request.url) {
        ...
        // 将 URL 存储
        $.setdata(data.url, KEY_erke_data);
        $.msg($.name, '', '🎉 URL 已抓取并保存');
    } else {
        console.warn('❌ 未捕获到请求 URL！');
...
function loadRequestData() {
    requestData = $.getdata(KEY_erke_data) || ''; // 从持久化存储读取
    if ($.isNode()) requestData = process.env.erke_data || requestData; // 兼容环境变量
    if (requestData) {
        console.log('🎉 加载到请求数据');
    } else {
...
async function executeForMultipleAccounts(data) {
    const accounts = data.split('@');  // 使用 @ 分隔多个账号的 URL
...
function buildSignInRequestBody(url) {
    ...
    return {
        ...
        wxOpenid: urlParams.get('wxOpenid'),
        enterpriseId: urlParams.get('enterpriseId'),
        appid: urlParams.get('appid'),
        gicWxaVersion: urlParams.get('gicWxaVersion'),
        timestamp: urlParams.get('timestamp'),
        cliqueMemberId: urlParams.get('cliqueMemberId'),
        unionid: urlParams.get('unionid'),
        source: 'wxapp',
        cliqueId: urlParams.get('cliqueId'),
        openid: urlParams.get('openid'),
        memberId: urlParams.get('memberId'),
        transId: urlParams.get('transId'),
        useClique: urlParams.get('useClique'),
