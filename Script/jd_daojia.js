          ...
          if (safeGet(data)) {
            ...
            if (data.code === '0') {
              if(data.result){
                ...
                console.log(`瓜分京豆团信息：${JSON.stringify({
                  activityId:activityId,
                  groupId:groupId
                })}`)
              }
...
function info(isInfo=false) {
  let body = {}
  return new Promise(resolve => {
    $.get(taskUrl("signin/showSignInMsgNew", body), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if(data.code==='0'){
              const {points,hasSign} = data.result.userInfoResponse
              if(isInfo){
                message += `本次运行获得${points-$.cur}鲜豆`
              }else{
                if(!hasSign)
                  await sign()
              }
...
function sign() {
  let body = {"modelId":"M10001","plateCode":1}
  return new Promise(resolve => {
    $.get(taskUrl("signin/userSigninNew", body), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
            ...
            if(data.code==='0'){
              for(let vo of data.result.taskInfoList){
                ...
                if(vo.status===2){
                  ...
                  await awardTask(vo.taskId,vo.taskType)
                }else if (vo.status===1 && ![506,513,511,509,512].includes(vo.taskType)){
...
function receiveTask(taskId,taskType) {
  let body = {"modelId":"M10001","plateCode":1,'taskId':taskId,'taskType':taskType}
  return new Promise(resolve => {
