...
[rewrite_local]
# 小米运动获取Token
https:\/\/account(\-cn2)?\.huami\.com(.*)\/login url script-response-body Script/backUpxmSports.js
Loon
[Script]
...
let login_token = '';
//需要修改的运动步数波动范围，脚本默认修改步数范围为30060到33010
const step = randomFriendPin($.getdata('xmMinStep')*1 || 30060, $.getdata('xmMaxStep')*1 || 33010);
function getToken() {
  if ($response.body) {
