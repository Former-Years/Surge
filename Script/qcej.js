...
let Message = '';
let token_Arr = [];
const KEY_qcej_token = 'qcej_token';
...
if (typeof $request !== 'undefined') {
    ...
    !(async () => {
        loadTokens(); // 加载 token
        if (!token_Arr.length) {
            $.msg($.name, '【提示】未找到有效的 token', '请先完成抓包或配置环境变量。');
            return;
        }

        for (let i = 0; i < token_Arr.length; i++) {
            const token = token_Arr[i];
            $.index = i + 1;
            console.log(`\n====== 开始执行账号 ${$.index} ======\n`);
            await executeForAccount(token);
        }
...
// 加载 token
function loadTokens() {
    let tokens = $.getdata(KEY_qcej_token) || ''; // 从持久化存储读取
    if ($.isNode()) tokens = process.env.qcej_token || tokens; // 兼容环境变量
    tokens = tokens.split('@').filter(Boolean); // 支持多个账号
    token_Arr = [...new Set(tokens)]; // 去重
    if (token_Arr.length) console.log(`🎉 加载到 ${token_Arr.length} 个 token`);
}

// 抓取 token
function GetTokenFromBody() {
    if ($request && $request.body) {
        const match = $request.body.match(/token=([^&]+)/);
        if (match) {
            const token = decodeURIComponent(match[1]);
            console.log(`🎉 提取到 token: ${token}`);
            let savedTokens = $.getdata(KEY_qcej_token) || '';
            const tokens = savedTokens.split('@').filter(Boolean);
            if (!tokens.includes(token)) tokens.push(token);
            $.setdata(tokens.join('@'), KEY_qcej_token); // 持久化
            $.msg($.name, '', '🎉 token 获取成功并已持久化');
        } else {
            console.warn('❌ 未找到 token，请检查抓包内容！');
            $.msg($.name, '', '❌ 未找到 token，请检查抓包工具是否正常运行。');
        }
    } else {
        console.warn('❌ 未捕获到请求体！');
        $.msg($.name, '', '❌ 未捕获到请求体，请检查抓包工具是否正常运行。');
    }
}

// 执行单账号任务
async function executeForAccount(token) {
    const accountMessage = [];
    const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
    let totalGoldBeans = 0; // 新增变量，记录本次运行的金豆总数

    try {
        // 签到
        const signInResult = await apiRequest('/bian/public/index.php/water/user/onSign', { token });
        accountMessage.push(`🎉 签到: ${signInResult || '成功'}`);

        // 看广告
        for (let i = 0; i < 3; i++) {
            const watchResult = await apiRequest('/bian/public/index.php/water/user/onShowAd', { token });

            // 提取金豆信息
            const match = watchResult.match(/金豆 \+(\d+)/);
            if (match) totalGoldBeans += parseInt(match[1], 10); // 累加金豆数

            await delay(1000); // 等待 1 秒，避免频繁请求
        }

        // 添加金豆统计
        accountMessage.push(`🎉 广告: 本次运行共金豆 +${totalGoldBeans}`);
    } catch (error) {
        console.error(`❌ 账号 ${$.index} 执行失败: ${error}`);
        accountMessage.push(`❌ 账号 ${$.index} 执行失败: ${error.message}`);
    } finally {
        Message += `账号 ${$.index}\n${accountMessage.join('\n')}\n\n`;
    }
}
...
function apiRequest(endpoint, body = {}) {
    const options = {
        ...
        body: JSON.stringify(body), // 将 token 放入请求体
    };
        ...
        $.post(options, (err, resp, data) => {
            if (err) {
                ...
                try {
                    ...
                    if (result && result.msg) resolve(result.msg);
                    else resolve('成功');
                } catch (error) {
