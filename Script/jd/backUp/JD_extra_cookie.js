...
===================
===================
使用方式：在代理软件配置好下方配置后，先后台关闭京东app，再打开京东app，自动获取 cookie，
...
===================
[MITM]
hostname = plogin.m.jd.com

===================Quantumult X=====================
[rewrite_local]
# 获取多账号京东Cookie
jd-mlogin\/static\/html\/appjmp_blank\.html url script-request-header JD_extra_cookie.js

===================Loon===================
[Script]
http-request jd-mlogin\/static\/html\/appjmp_blank\.html script-path=JD_extra_cookie.js, tag=获取多账号京东Cookie

===================Surge===================
[Script]
获取多账号京东Cookie = type=http-request,pattern=jd-mlogin\/static\/html\/appjmp_blank\.html,requires-body=1,max-size=0,script-path=JD_extra_cookie.js,script-update-interval=0
 */
...
function GetCookie() {
  try {
    if ($request.headers && $request.url.indexOf("jd-mlogin") > -1) {
      var CV = $request.headers["Cookie"] || $request.headers["cookie"];
      if (CV.match(/(pt_key=.+?pt_pin=|pt_pin=.+?pt_key=)/)) {
        if (!CV.match(/pt_key=app_open.+?;/)) return
